...
function check_login()
{
	...
	for ($i=1;$users[$i];$i++)
	{
		...
		{	
			$indir = getcookie("indir")? getcookie("indir").'/':"";
			$addroot = getcookie("getroot");
			//$dd["root"] = $arr[2];
			$dd["root"] = $addroot.$indir;
			$dd["name"] = $arr[0];
			$dd["group"] = $arr[3];
			$rights = $group["{$arr[3]}"];
...
function unpackzip( $filename, $key, $indexes = -1,$cover = 0, &$info)
{
	...
	foreach($l as $file)
	{
		...
		{
			if (file_exists($path)) // 錕斤拷錕斤拷錕窖撅拷錕斤拷錕節碉拷錕僥鹼拷
			{
