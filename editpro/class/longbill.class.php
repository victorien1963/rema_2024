...
class longbill
{
	...
	var $ftp_pass;
	var $ftp_http_path;
	var $searchfiles; //$searchfiles為可搜索檔案的類型,比如 "asp|php|txt";
	
	function del_dir($path,&$info,&$err)  //強制刪除檔案夾
	{
		global $user;
		$info = array();
		$info["file"]= 0;
		...
		foreach($files as $f)
		{
			...
			if ($user["limit"]["$ftype"] && !$user["only"])
				$err.="檔案{$f}刪除失敗:不能刪除{$user["limittype"]}類型的檔案!";
			else if (!$user["limit"]["$ftype"] && $user["only"])
				$err.="檔案{$f}刪除失敗:不能刪除除{$user["limittype"]}類型以外的檔案!";
	...
	function dir($path,&$dir,&$file,&$size,$deep=0) //讀取目錄和檔案
	{
	...
	function pre_search() 
	{
		if (!$this->searchfiles)//如果沒有自己配置$searchfiles 則搜索以下檔案類型
		{
	...
	function search_file($path,$instr,$type=0,$case=false) //$instr 為包含的字元串 $type=0,1,2 分別表示 搜索全部，只搜檔案名，只搜檔案內容 $case表示區分大小寫,
	{
