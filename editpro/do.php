...
if ( $action == "savefile" && $_POST['encode'] != $host_charset)
{
	$i = @iconv($_POST['encode'],$host_charset,$path);
	if ($i) $path = $i;
}
...
if (!is_file($path) && !is_dir($path) && $action != "savefile") exitme("alert(lang.not_found+' $path');opendir('$root');","eval");
...
else if ($action == "savefile")
{
	...
	//$txt_main = str_repeat($configroot.'images/','{#IMAGES#}',StripSlashes($txt_main));
	$content = StripSlashes($txt_main);
	...
	if ($_POST['encode'] != $_POST['encodeto'])
	{
		...
		$out.= $_POST['encode'].'->'.$_POST['encodeto'].'\n';
	}
	else if ($err) $out = basename1($filename).'\n';
	...
	else if ($_POST['encode'] != $_POST['encodeto'])
	{
		echo "alert('$out');";
	}
...
else if ($action=="paste")
{
	...
	if ($frompath == $path)
	{
		...
		for($i=0;$i<count($sdir);$i++)
		{
			if (!$sdir[$i]) continue;
			$k=1;
			$filename=$path."復件 ".$sdir[$i];
			while (file_exists($filename))
			{
				$filename=$path."復件(".$k.") ".$sdir[$i];
				$k++;
			}			
...
else if ($action == "property" )
{
	...
	$s.= "+lang.br+lang.br+'<br/><br/><a href=\"javascript:search_close();\">關閉</a>'";
