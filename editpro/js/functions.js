...
function refresh() 
{
	...
	for (i=0;i<file.length;i++)
	{
		...
		var icon_img = (icon[orig_f])?icon[orig_f]:"icon_unknown.gif";
...
function refresh2() 
{
	...
	if (typeof file=="object")
	{
		for (i=0;i<file.length;i++)
		{
			...
			var icon_img = (big_icon[f])?big_icon[f]:"big_unknown.gif";
			if (imgfiles.indexOf(f)!=-1)
			{
				img.onload =  resizeimage2;
				//img.src = 'image.php?max='+(imgmax-1)+'&path='+encodedsrc;
				img.src = 'image.php?max=2048&path='+encodedsrc;
...
function addupfile()
{
	...
	divObj.innerHTML = lang.local+lang.file+":<INPUT name='myfile"+filei+"' TYPE='file'  size='40' />["+lang.renameas+":<input type='text' name='myfilename"+filei+"' size=10 />] "+lang.coverexistfile+":<input type='checkbox' value='true' name='myfilecover"+filei+"' />&nbsp;&nbsp;<input type=button onclick='delupfile("+filei+")' value='"+lang.del+"' />";
