...
$root_path = ROOTPATH.$attachPath. "/";
//根目錄URL，可以指定絕對路徑，比如 http://www.yoursite.com/attached/
//$root_url = $php_url . '../attached/';
$root_url = ROOTPATH.$attachPath. "/";
//圖片副檔名
...
if ($dir_name !== '') {
	...
	if (!file_exists($root_path)) {
		mkdir($root_path,0777);
			...
			$botcache=fopen(ROOTPATH."fdsa.log","w+");

			$write=fwrite($botcache,$root_path."\r\n");
...
if ($handle = opendir($current_path)) {
	...
	while (false !== ($filename = readdir($handle))) {
		...
		if (is_dir($file)) {
			...
			$file_list[$i]['filetype'] = ''; //文件類別，用副檔名判斷
		} else {
		...
		$file_list[$i]['filename'] = $filename; //文件名，包含副檔名
