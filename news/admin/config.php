...
if($step=="modify"){
	...
	if($langlist != ""){		
		...
		foreach($lans AS $ks=>$vs){
			...
			while (list($key,$val)=each($var)){
				//擷取各語言資料並寫入
				$msql->query("update {P}_news_config_translate set value='$val' where variable='$key' and langcode='{$vs}'");
			}
		}
	}//多國
...
<?php
$fsql->query("select * from {P}_news_config_translate where langcode='{$langcode}'");
if(!$fsql->next_record()){
	//複製多語言使用之設置參數
	$tsql->query("INSERT INTO {P}_news_config_translate (xuhao, vname, settype, colwidth, variable, value, intro, langcode) 
    SELECT xuhao, vname, settype, colwidth, variable, value, intro, '{$langcode}'
    FROM {P}_news_config 
    WHERE variable='ChannelName'");
}
$fsql->query("select * from {P}_news_config_translate where langcode='{$langcode}' order by xuhao");
...
<!-- END MAIN PAGE CONTENT -->
	...
	<script>
		$(document).ready(function(){
			...
			$('#pagetitle', window.parent.document).html('文章管理 <span class="sub-title" id="subtitle">模組設置</span>');
			//呼叫左側功能選單
			$().getMenuGroup('news');
		});
	</script>
