...
//電子報發佈表單送出
$(document).ready(function(){
	$('#paperfabu').submit(function(){ 
		SelectAll('spe_selec[]', 'select[]');
		$('#paperfabu').ajaxSubmit({
			target: 'div#notice',
			url: 'post.php',
			success: function(msg) {
				if(msg=="OK"){
					$('div#notice').hide();
					$().alertwindow("發佈成功","paper_gl.php");
				}else{
...
$(document).ready(function(){
	...
	$('#paperModify').submit(function(){ 

		if($("#paperpagesid")[0].value=="0"){
			
			//正常送出修改
			$("#act")[0].value="papermodify";
			SelectAll('spe_selec[]', 'select[]');

			$('#paperModify').ajaxSubmit({
				target: 'div#notice',
				url: 'post.php',
				success: function(msg) {
					if(msg=="OK"){
						$('div#notice').hide();
						$().alertwindow("修改成功","paper_gl.php");
					}else{
						$('div#notice')[0].className='noticediv';
						$('div#notice').show();
						$().setBg();
...
(function($){
	$.fn.getNewsPages = function(p){
		...
		$("div#paperpages").empty();
		
		var nowid=$("#nowid")[0].value;
		$.ajax({
			type: "POST",
			url:"post.php",
			data: "act=paperpageslist&nowid="+nowid+"&pageinit="+p,
			success: function(msg){
				$("div#paperpages").append(msg);
				
				var nowpagesid=$("input#paperpagesid")[0].value;
				...
				$("li#pagedelete").click(function(){
					...
					var delpagesid=$("input#paperpagesid")[0].value;
...
(function($){
	$.fn.getContent = function(paperpageid){
		...
		$.ajax({
			type: "POST",
			url:"post.php",
			data: "act=getbody&paperpageid="+paperpageid+"&nowid="+nowid,
			success: function(msg){
