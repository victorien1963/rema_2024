...
if($step=="copy"){
		...
		//多語言
		$fsql->query( "delete from {P}_paper_prop_translate where pid='{$catid}'" );
		$p=1;
		$msql->query("select * from {P}_paper_prop where catid='$pid'");
		while($msql->next_record()){
			$projid = $msql->f( "id" );
			$propname=$msql->f("propname");
			$xuhao=$msql->f("xuhao");

			$fsql->query("insert into {P}_paper_prop values(
			0,
			'$catid',
			'$propname',
			'$xuhao'
			...
			$wsql->query( "select * from {P}_paper_prop_translate where pid='{$projid}'" );
			while ( $wsql->next_record( ) )
			{
				$propname = $wsql->f( "propname" );
				$langcode = $wsql->f( "langcode" );
				$fsql->query( "insert into {P}_paper_prop_translate values(
				0,
				'{$getprojid}',
				'{$langcode}',
				'{$propname}'
...
if ( $step == "copytoall" )
{
				$msql->query( "select catid from {P}_paper_cat" );
				while ( $msql->next_record( ) )
				{
					$getcatid = $msql->f( "catid" );
					if($getcatid != $catid){
						$fsql->query( "delete from {P}_paper_prop where catid='{$getcatid}'" );
						//多語言
						$fsql->query( "delete from {P}_paper_prop_translate where pid='{$getcatid}'" );

						$tsql->query( "select * from {P}_paper_prop where catid='{$catid}'" );
...
if ( $step == "copytosub" )
{
		...
		$msql->query( "select * from {P}_paper_cat where pid='{$catid}'" );
		while ( $msql->next_record( ) )
		{
				$subcatid = $msql->f( "catid" );
				$fsql->query( "delete from {P}_paper_prop where catid='{$subcatid}'" );
				//多語言
				$fsql->query( "delete from {P}_paper_prop_translate where pid='{$catid}'" );
				$i = 0;
				for ( ;	$i < $nums;	$i++	)
				{
						$p = $arr['propname'][$i];
						$x = $arr['xuhao'][$i];
						$tsql->query( "insert into {P}_paper_prop set
							`catid`='{$subcatid}',
							`propname`='{$p}',
							`xuhao`='{$x}'
						" );
...
if($step=="add"){
	$msql->query("select count(id) from {P}_paper_prop where catid='$catid'");
	if($msql->next_record()){
	...
	$msql->query("select max(xuhao) from {P}_paper_prop where catid='$catid' ");
	if($msql->next_record()){
		$max=$msql->f('max(xuhao)');
	}

	$max=$max+1;
	$msql->query("insert into {P}_paper_prop values(
	0,
	'$catid',
	'$propname',
	'$max'
...
if($step=="modify"){
	$msql->query("update {P}_paper_prop set propname='$propname',xuhao='$xuhao' where id='$id'");
//記錄多國翻譯資料
	...
	if($langlist != ""){			
		...
		foreach($lans AS $ks=>$vs){
			...
			$propnames = htmlspecialchars($spropname[$vs]);
				...
				$msql->getupdate(
					"SELECT id FROM {P}_paper_prop_translate WHERE pid='{$id}' AND langcode='{$vs}'",
					"UPDATE {P}_paper_prop_translate SET 
					propname='{$propnames}' WHERE pid='{$catid}' AND langcode='{$vs}'",
					"INSERT INTO {P}_paper_prop_translate SET 
					pid='{$catid}',
					langcode='{$vs}',
					propname='{$propnames}'"
				);
...
if($step=="del"){
	$msql->query("delete from {P}_paper_prop where id='$id'");
	$msql->query("delete from {P}_paper_prop_translate where pid='$id'");
}
					...
					<td class="col-width">
						...
						<?php
							...
							while ( $fsql->next_record( ) )
							{
								...
								//依表擷取語言檔內容
								$langs = $tsql->getone( "SELECT * FROM {P}_paper_prop_translate WHERE pid='{$id}' AND langcode='{$langcode}'" );
						?>
							<div class="form-group">
