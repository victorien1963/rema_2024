...
if($step=="add" && $_REQUEST["cat"]!="" && $_REQUEST["cat"]!=" "){
	...
	if($pid!="0"){
		$msql->query("select catpath from {P}_paper_cat where catid='$pid'");
		if($msql->next_record()){
			$pcatpath=$msql->f('catpath');
		}
	}

	$msql->query("select max(xuhao) from {P}_paper_cat where pid='$pid'");
		if($msql->next_record()){
			$maxxuhao=$msql->f('max(xuhao)');
			$nowxuhao=$maxxuhao+1;
		}

	$msql->query("insert into {P}_paper_cat set
	`pid`='$pid',
	`cat`='$cat',
	`xuhao`='$nowxuhao',
	`catpath`='$catpath',
	...
	$msql->query("update {P}_paper_cat set catpath='$catpath' where catid='$nowcatid'");
...
if($step=="del"){
	...
	$msql->query("select id from {P}_paper_con where catid='$catid' ");
	if($msql->next_record()){
		err($strPaperNotice1,"","");
		exit;
	}
	$msql->query("select catid from {P}_paper_cat where pid='$catid'");
	if($msql->next_record()){
		err($strPaperNotice2,"","");
		exit;
	}

	$msql->query("select ifchannel from {P}_paper_cat where catid='$catid'");
	if($msql->next_record()){
	...
	$msql->query("delete from {P}_paper_cat where catid='$catid'");
	$msql->query("delete from {P}_paper_cat_translate where pid='$catid'");
	//刪除參數(含多語言)
	$msql->query("select * from {P}_paper_prop where catid='$catid'");
	while($msql->next_record()){
		$ppid=$msql->f('id');
		$fsql->query("delete from {P}_paper_prop_translate where pid='$ppid'");
	}
	$msql->query("delete from {P}_paper_prop where catid='$catid'");
}
...
if($step=="modi"){
	...
	$msql->query("update {P}_paper_cat set cat='$cat',xuhao='$xuhao' where catid='$catid' ");
	$msql->query("update {P}_paper_cat set tj='$tj' where catpath regexp '".fmpath($catid)."' ");
	...
	if($langlist != ""){			
		...
		foreach($lans AS $ks=>$vs){
			...
			$cats = htmlspecialchars($scat[$vs]);
				...
				$msql->getupdate(
					"SELECT id FROM {P}_paper_cat_translate WHERE pid='{$catid}' AND langcode='{$vs}'",
					"UPDATE {P}_paper_cat_translate SET 
					cat='{$cats}' WHERE pid='{$catid}' AND langcode='{$vs}'",
					"INSERT INTO {P}_paper_cat_translate SET 
					pid='{$catid}',
					langcode='{$vs}',
					cat='{$cats}'"
				);
			...
			<div id="f-1" class="panel-collapse collapse in">
				<div class="portlet-body">
					<form id="selgroup" name="selgroup" method="get" action="" class="form-inline pull-left" role="form">
						<div class="form-group">
							<select name="pid" class="form-control" onChange="self.location=this.options[this.selectedIndex].value">
							  <option value='paper_cat.php'><?php echo $strPaperSelCat; ?></option>
						         <?php
										...
										while ($fsql -> next_record ()) {
											...
											if($pid==$lcatid){
												echo "<option value='paper_cat.php?pid=".$lcatid."' selected>".$ppcat.$cat."</option>";
											}else{
												echo "<option value='paper_cat.php?pid=".$lcatid."'>".$ppcat.$cat."</option>";
											}
											$ppcat="";
										}
								 ?>
					...
					<div class="pull-right">
						<form name="addcat" method="get" action="paper_cat.php" class="form-horizontal" onSubmit="return catCheckform(this)">
						...
						<div class="fleft" style="margin: 0 5px;">
							<input name="cat" type="text" class="form-control" placeholder="<?php echo $strPaperCatName; ?>" value="" />
						</div>
						<div class="fleft">
				...
				<table class="table table-bordered table-hover tc-table tc-gallery">
					<thead>
						<tr>
							...
							<th class="col-mini center"><?php echo $strPaperList6; ?></th>
					...
					<tbody>
<?php
	$msql->query("select * from {P}_paper_cat where  pid='$pid' order by xuhao");
	while($msql->next_record()){
		$catid=$msql->f("catid");
		$cat=$msql->f("cat");
		$xuhao=$msql->f("xuhao");
		...
		if($ifchannel=="1"){
			$href="../class/".$catid."/";
			$url="paper/class/".$catid."/";
		}else{
			$href="../class/?".$catid.".html";;
			$url="paper/class/?".$catid.".html";
		}
?> 
						<tr>
							<form method="get" action="paper_cat.php" class="form-horizontal">
										...
										<div id="l_<?php echo $catid; ?>" class="panel-collapse collapse">
											...
											<?php
												...
												while ( $fsql->next_record( ) )
												{
													...
													//依表擷取語言檔內容
													$langs = $tsql->getone( "SELECT * FROM {P}_paper_cat_translate WHERE pid='{$catid}' AND langcode='{$langcode}'" );
											?>
												<div class="form-group">
								...
								<td class="col-small center">
									<img src="images/prop.png" border=0 style="cursor:pointer;display:<?php echo $listdis; ?>" onClick="callcolorbox('../../paper/admin/prop_frame.php?catid=<?php echo $catid; ?>&pid=<?php echo $pid; ?>')">
								</td>
								<td class="col-mini center">
									<img src="images/delete.png"  style="cursor:pointer"  border=0 onClick="self.location='paper_cat.php?step=del&catid=<?php echo $catid; ?>&pid=<?php echo $pid; ?>'">
								</td>
							</form>
						</tr>
<?php
...
<!-- END MAIN PAGE CONTENT -->
	...
	<script>
		$(document).ready(function(){
			...
			$('#pagetitle', window.parent.document).html('電子報管理 <span class="sub-title" id="subtitle">電子報分類管理</span>');
			//呼叫左側功能選單
			$().getMenuGroup('paper');
		});
	</script>
