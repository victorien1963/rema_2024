	  ...
	  if ($this->io_method() == 'socket') {
		...
		if($connection_timeout > 0){
			if(substr(phpversion(),0,1)=="4"){
				...
				$this->fp = @stream_socket_client($host.":".$this->port, $this->errno, $this->error_str, $connection_timeout);
			}
		} else {
			if(substr(phpversion(),0,1)=="4"){
				$this->fp = @fsockopen( $host, $this->port, $this->errno, $this->error_str);
			}else{
				$this->fp = @stream_socket_client($host.":".$this->port, $this->errno, $this->error_str);
			}
		}
...
class soapclientx extends nusoap_base  {
	...
	* @access   public
	*/
	function soapclientx($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 180, $portName = ''){
		parent::nusoap_base();
	...
	* @access   private
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=180) {
		$this->checkCookies();
		// detect transport
		switch(true){
			// http(s)
	...
	*/
	...
	";
		...
		$evalStr = 'class nusoap_proxy_'.$r.' extends soapclientx {
	'.$evalStr.'
}';
		return $evalStr;
	}
...
if (!extension_loaded('soap')) {
	 ...
	 */
	class soapclient extends soapclientx {
	}
}
?>
