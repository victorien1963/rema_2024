...
/* 備份數據庫 */ 
$alltables = $msql->query("show tables from ".$msql->Database);
$getname = "Tables_in_".$msql->Database;
while($res = $msql->fetch_array($alltables)){	
	$tables[] = $res[$getname];
}

//$tables = array('dev_base_pageset','dev_base_plus'); //定義要保存的數據表，一個數組 
$prefix = '_data'; // 要保存的.sql文件的前綴 
$saveto = 'server'; // 要保存到什麼地方，是本地還是服務器上，默認是服務器
$db_backup_path = '../../cache/datasql/';
$back_mode = 'all'; // 要保存的方式，是全部備份還是只保存數據庫結構 
$admin = $file_path; //管理員名稱 
$admin_email = $SiteUrl; // 管理員郵箱 
// 定義數據保存的文件名 
