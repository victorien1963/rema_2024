...
class CustomerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //新增發票資訊
        $invoLoveCode=""; $invoBarcode=""; $invoTitleNo="";
        ...
        //三聯電子發票
        if( !is_null($Order[0]->invoicenumber)){
            $invoTitleNo = $Order[0]->invoicenumber;
        }
        
        $data["datas"][0]["invoLoveCode"] = $invoLoveCode;//發票愛心捐贈碼
        $data["datas"][0]["invoBarcode"] = $invoBarcode;//發票手機載具
        $data["datas"][0]["invoTitleNo"] = $invoTitleNo;//發票統編
        $data["datas"][0]["mail"] = $Order[0]->email;
    ...
    function upd_order_cancel(Request $request){
        ...
        try{
            ...
            $lurl = "https://requestinspector.com/inspect/tuitui";
            //補資料
            $OrderID = $request->input('orderid');
            $Order = Order::where('orderid', $OrderID)->get();
                ...
                if($data['errorCode'] == 0){
                    if(!$data['datas'] == null){
                        ...
                        foreach($data['datas'] as $datas){
                            ...
                            $orderableNum = $datas['orderableNum'];
                        }
                        //更新目前的總庫存量
    ...
    function get_stock(Request $request){
            ...
            try{
                if(strstr($Members->posproid, '-')){
                    ...
                    if($data['errorCode'] == 0){
                        ...
                        ApiConfig::create( [ 'name' => 'get_stock', 'input_data' => "$max-$mix" , 'status' => '0', 'message'=> 'api error'] );
                        Log::error($data);  
                    }
                }
            }catch(\Exception $e){
        ...
        try {
            ...
            ApiConfig::create( [ 'name' => 'get_stock', 'input_data' => "$max-$mix" , 'status' => '0', 'message'=> 'insert DB finish'] );
        } catch(\Exception $e) {
            ApiConfig::create( [ 'name' => 'get_stock', 'input_data' => "$max-$mix" , 'status' => '0', 'message'=> 'insert DB error'] );
            Log::error($e->getMessage());
        }
    }
    ...
    function cre_order(Request $request){
        ...
        $data  = $this->create_dataArray("upOrderData");
        $url = "http://rm501.wpos.com.tw:8899/rfsys/odid/up";
        $lurl = "https://requestinspector.com/inspect/dingding";
        //補資料
        $OrderID = $request->input('orderid');
        $Order = Order::where('orderid', $OrderID)->get();
        $OrderItems = OrderItems::where('orderid', $OrderID)->get();
