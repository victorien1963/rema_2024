...
$(document).ready(function(){
	$(".fixspec").click(function(){
				...
				$.ajax({
					type: "POST",
					url:"post.php",
					data: "act=fixspec&specid="+specid+"&f_size="+f_size+"&f_sprice="+f_sprice+"&f_stocks="+f_stocks+"&f_colorname="+f_colorname+"&f_colorcode="+f_colorcode+"&f_iconsrc="+f_iconsrc,
					success: function(msg){
						if(msg=="OK"){
							$().alertwindow("修改完成","");
						}else{
...
$(document).ready(function(){
	...
	/*寫入排序 2017-02-06*/
	$(".chgorixuhao").on("blur change",function(){ 
		var shopid = this.id.substr(9);
		var xuhao = $(this).val();
		$.ajax({
			type: "POST",
			url:"post.php",
			data: "act=chgorixuhao&shopid="+shopid+"&xuhao="+xuhao,
			success: function(msg){
				if(msg=="OK"){
					$().gritter("OK","");
				}else{
	...
	$(".chgsortxuhao").on("blur change",function(){ 
		var sort = this.id.split("_");
		var xuhao = $(this).val();
		$.ajax({
			type: "POST",
			url:"post.php",
			data: "act=chgsortxuhao&sortid="+sort[1]+"&gid="+sort[2]+"&catid="+sort[3]+"&xuhao="+xuhao,
			success: function(msg){
				var msgs = msg.split("_");
				if(msgs[0]=="OK"){
					$().gritter("OK","");
					if(msgs[1] != ""){
						$("#sort__"+sort[2]+"_"+sort[3]).prop("id","sort_"+msgs[1]+"_"+sort[2]+"_"+sort[3]);
					}
				}else{
					alert(msg);
					$().gritter(msg,"");
