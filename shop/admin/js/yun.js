...
$(document).ready(function(){
	...
	$("#addYunZone").click(function(){
		...
		if($("input#newzone")[0].value=="" || $("input#newzone")[0].value=="請輸入一級區域名稱"){
			alert("請輸入一級區域名稱");
		}else{
			...
			$.ajax({
				...
				success: function(msg){
					if(msg.substr(0,2)=="OK"){
						...
						$("div#bigcat_"+bigzoneid).append("<input type='button' class='button_zone_del' id='topZoneDel_"+bigzoneid+"' value='刪除' /> ");
						$("div#bigcat_"+bigzoneid).append("<input type='button' class='button_zone_open' id='topZoneOpen_"+bigzoneid+"' value='展開二級區域' /> ");

						$("input#newzone")[0].value="請輸入一級區域名稱";
...
function openSubZone(bigzoneid){
		if($("#topZoneOpen_"+bigzoneid)[0].value=="展開二級區域"){
			$.ajax({
				type: "POST",
				url:"post.php",
				...
				success: function(msg){
					if(msg!=""){
						...
						$("#topZoneOpen_"+bigzoneid)[0].value="隱藏二級區域";
						...
						$(".button_subzone_add").unbind().bind('click',function(){
							...
							if($("input#newsubzone_"+newsubpid)[0].value=="" || $("input#newsubzone_"+newsubpid)[0].value=="請輸入二級區域名稱"){
								alert("請輸入二級區域名稱");
							}else{
								...
								$.ajax({
									...
									success: function(msg){
										if(msg.substr(0,2)=="OK"){
											...
											$("div#subcat_"+subzoneid).append("<input type='button' class='button_subzone_del' id='subZoneDel_"+subzoneid+"' value='刪除' /> ");

											$("input#newsubzone_"+newsubpid)[0].value="請輸入二級區域名稱";
...
function closeSubZone(bigzoneid){
		$("#topZoneOpen_"+bigzoneid)[0].value="展開二級區域";
		$("#suball_"+bigzoneid).remove();
}
...
$(document).ready(function(){
	$("#showzonebutton").click(function(){
		...
		$('#frmWindow').append('<div class="topBar">選擇適用區域<div class="pwClose"></div></div><div class="border"><iframe frameborder="0" scrolling="yes" src="yun_selzone.php" class="Frm"></iframe></div>');
