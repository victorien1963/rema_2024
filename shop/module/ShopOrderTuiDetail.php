...
function ShopOrderTuiDetail(){
	...
	if($_POST["act"] && $_POST["act"]=="sendtui"){
		if($_POST["tuilist"] != ""){
			foreach($_POST["tuilist"] AS $vv){
				$tuilist .= $tuilist? ",".$vv:$vv;
			}
			$sltui = "AND id IN($tuilist)";
		}
	}elseif($_GET["act"] && $_GET["act"]=="ok"){
		$str=$TempArr["menu"];
		return $str;
		exit();
	}
	...
	if($msql->next_record()){
		...
		$ip=$msql->f('ip');
		$dtime=$msql->f('dtime');
		$yuntime=$oriyuntime=$msql->f('yuntime');
		$paytime=$msql->f('paytime');
		$bz=$msql->f('bz');
		$items=$msql->f('items');
		
		$orderitemtui = $msql->f( "itemtui" );
	...
	if($_COOKIE["SYSUSER"]!=""){
		$betime = $oriyuntime + ((86400*365)-1);
	}else{
		$betime = $oriyuntime + ((86400*11)-1);
	}
	...
	'items' => $items,
		...
		'statupic' => $statupic,
		'showtui' => $showtui,
		'preid' => $sid[0]!=""? ROOTPATH."shop/ordertuidetail.php?orderid=".$sid[0]:"javascript:;",
		'nextid' => $sid[1]!=""? ROOTPATH."shop/ordertuidetail.php?orderid=".$sid[1]:"javascript:;",
		'account' => $account,
	...
	$msql->query("select * from {P}_shop_orderitems where orderid='$orderid' $sltui");
	while($msql->next_record()){
		...
		/*echo(date("Y-m-d H:i:s",$oriyuntime));
		echo "<br>";
		echo(date("Y-m-d H:i:s",$betime));
		exit();*/
		
		if( time() > $betime || $orderitemtui == "1"){ 
			$dischk = "display:none";
		}
		...
		/*擷取不可退貨商品*/
		$arrnoretuen = explode(",",$GLOBALS['GLOBALS']['SHOPCONF']['noReturnCode']);
		if( in_array($bn,$arrnoretuen) ){
			$dischk = "display:none";
		}
