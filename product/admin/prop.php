...
if($step=="copy"){
		...
		while($msql->next_record()){
			...
			$fsql->query("insert into {P}_product_prop values(
			0,
			'$catid',
			'$propname',
			'$xuhao'
...
if($step=="add"){
	$msql->query("select count(id) from {P}_product_prop where catid='$catid'");
	if($msql->next_record()){
...
$msql->query("select max(xuhao) from {P}_product_prop where catid='$catid' ");
if($msql->next_record()){
$max=$msql->f('max(xuhao)');
}

$max=$max+1;
$msql->query("insert into {P}_product_prop values(
0,
'$catid',
'$propname',
'$max'
...
if($step=="modify"){
$msql->query("update {P}_product_prop set propname='$propname',xuhao='$xuhao' where id='$id'");
}
if($step=="del"){
$msql->query("delete from {P}_product_prop where id='$id'");
}
